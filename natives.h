...
//extern void ATTACH_PED_TO_CAR(Ped ped, Vehicle vehicle, uint unknown0_0, float offsetX, float offsetY, float offsetZ, float rotX, float rotY);
...
extern uint CREATE_CHECKPOINT(int type, float x, float y, float z, float, float, float, float);
...
extern void ADD_NEW_CONVERSATION_SPEAKER(int id, Ped ped, char *name);
...
extern void ADD_LINE_TO_CONVERSATION(int id, char *aud, char *gxt, int b, int c);
...
extern boolean GET_CLOSEST_ROAD(float x, float y, float z, float minLength, int minLanes, Vector3 *SouthEndNode, Vector3 *NorthEndNode, int *, int *, float *);
...
extern void TASK_CAR_DRIVE_TO_COORD(int unk, int car, float x, float y, float z, float r, int a, int b, int c, float d, int e);
...
extern void TASK_PLAY_ANIM_SECONDARY_UPPER_BODY(Ped ped, char *animname0, char *animname1, float, int, int, int, int, int);
